...
namespace RB2MoneyEditor2025.x360
{
    ...
    public static class AssemblyFunctions
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static string GrabParentProcessName() { try { return Process.GetProcessById((int)(new PerformanceCounter("Process", "Creating Process ID", Process.GetCurrentProcess().ProcessName).NextValue())).ProcessName; } catch { return null; } }
    ...
    public static class VariousFunctions
    {
        ...
        {
            var xid = ID.ToString(CultureInfo.InvariantCulture);
            xid = xid.Substring(0, xid.Length - 1);
            var xcrc = xid.Aggregate<char, byte>(0, (current, x) => (byte) ((byte.Parse(x.ToString(CultureInfo.InvariantCulture)) + current)%10));
        ...
        {
            var xid = (ConsoleID >> 4).ToString(CultureInfo.InvariantCulture);
            var xcrc = xid.Aggregate<char, byte>(0, (current, x) => (byte) ((byte.Parse(x.ToString(CultureInfo.InvariantCulture)) + current)%10));
            return long.Parse(xid + xcrc.ToString(CultureInfo.InvariantCulture));
        }
        ...
        {
            if (string.IsNullOrWhiteSpace(x)) throw STFSExcepts.InvalChars;
        ...
        public static XboxFileType ReadFileType(string FileLocale)
        {
            ...
            try
            {
                ...
                switch (sig)
                {
                    ...
                    case (uint) AllMagic.XDBF:
                    {
                        xReturn = XboxFileType.GPD;
                    }
                        break;
        ...
        public static string GetFriendlySize(long xSize)
        {
            ...
            return (size / 1024.0).ToString("#.00") + " TB";
        }
    }
    ...
    public static class Imaging
    {
        ...
        {
            var xMS = new MemoryStream(xBytes);
            var xReturn = Image.FromStream(xMS);
            xMS.Dispose();
            return xReturn;
        }
