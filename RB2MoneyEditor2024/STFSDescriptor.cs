...
namespace RB2MoneyEditor2025.x360
{
    ...
    internal class BlockRecord
    {
        ...
        readonly byte[] xThisBlock = new byte[3];
        ...
        public BlockRecord(HashStatus xStatus, uint xNext) { Flags = (uint)xStatus << 30 | (xNext & 0xFFFFFF); }
    ...
    public sealed class STFSDescriptor
    {
        ...
        [CompilerGenerated]
        readonly uint[] xSpaceBetween = new uint[3];
        [CompilerGenerated]
        readonly byte xBaseByte;
        [CompilerGenerated]
        ...
        static uint[] BlockLevel { get { return Constants.BlockLevel; } }
        ...
        internal uint GenerateDataBlock(uint xBlock)
        {
            ...
            try
            {
                ...
                var num = ((((xBlock / BlockLevel[0]) + 1) << Shift) + xBlock);
                    ...
                    case TreeLevel.L0:
                        {
                            ...
                            var num = (xBlock / BlockLevel[0]) * SpaceBetween[0];
                            // Adjusts the result for Level 1 table count
                            if (xBlock < BlockLevel[0]) return num;
                            num += (((xBlock / BlockLevel[1]) + 1) << Shift);
        ...
        internal long GenerateHashOffset(uint xBlock, TreeLevel xTree)
        {
            ...
            try
            {
                var result = GenerateHashBlock(xBlock, xTree);
                var xReturn = BlockToOffset(result);
                switch (xTree)
                {
        ...
        internal long GenerateDataOffset(uint xBlock)
        {
            try
            {
                return xBlock >= 0x4AF768 ? Constants.STFSEnd : BlockToOffset(GenerateDataBlock(xBlock));
            }
            catch { throw STFSExcepts.General; }
        }
